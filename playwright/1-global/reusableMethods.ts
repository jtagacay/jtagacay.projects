...
export class reusableMethods {
    ...
    */
    ...
    // async getLogs() {
    //     console.log(`\nRetrieving logs...`)
    //     console.log(`Total logs collected: ${this.logBuffer.length}`)
     ...
     * @param remarks - input any message or remarks or element name in this parameter
     */
    async verifyElementIsHidden(locator:Locator, remarks?:string) {
        ...
        try {
            if(value === 'getValue') {
                ...
                this.generateConsoleLog(
                    ...
                    : `Failed| Attribute| Element - attribute was not matched\n ● ${RED}Attribute: ${attribute}${reset} >>> ${RED}Value: ${value}${reset}| ${remarks}`
                )
            }
        }
        catch(error) {
    ...
    async dragAndDropElement(locator: Locator, dragX: number, dragY: number, remarks?: string, dragTo?: Locator) {
        ...
        try {
            let curPositionX: any = box?.x
            let curPositionY: any = box?.y
            ...
            let newPositionX: any = newBox?.x
            let newPositionY: any = newBox?.y
